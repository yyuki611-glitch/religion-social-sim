...
4. 静的チェック: README 内の図パスが実ファイルと一致（grep/ls で確認）。GitHub 上での図表示の目視確認は push 後に 作者 へ依頼
...
- evaluator モード2で完了基準1–3・5–6 を検証（4のGitHub目視のみ 作者 依頼）
...
4. PR 作成 → merge（作者 承認済み「進めていきましょう」、public repo 既承認）
